...
BODY = r'''# -*- coding: utf-8 -*-
"""hermes-verify-hjnj-ch1-5.py — qhzy-黄帝内经 1-5章正文（原文+注释）聚焦验证（ad-hoc）"""
...
for s, (n_pian, pnames) in expected.items():
    ...
    check(f"{s}原文≥15段", cats['原文'] >= 15, f"实际{cats['原文']}")
    check(f"{s}注释≥4段", cats['注释'] >= 4, f"实际{cats['注释']}")
    for pn in pnames:
        check(f"{s}含篇[{pn}]", any(pn in g for g in got))
...
for s in ch:
    ...
    check(f"{s}注释含非零和", '非零和' in zt or ('并存' in zt and '注' in zt))

# 5. 0目录 未破坏（素问1-20篇在目录中）
tt = "\n".join(str(c.value) for row in wb['0目录'].iter_rows() for c in row if c.value)
check("0目录素问篇完整", '上古天真论' in tt and '三部九候论' in tt and '痈疽' in tt)
...
fd, tmp_path = tempfile.mkstemp(prefix='hermes-verify-hjnj-ch1-5-', suffix='.py', dir=tempfile.gettempdir())
