# -*- coding: utf-8 -*-
"""
Step C: qhzy-方剂学 引擎Sheet 增补 + 0改写说明升级 + 平台建设与依赖增补
注意：方剂学引擎 Sheet 结构与前几本不同（多智能体协同13行/思维流28行/Skills18行含扩展说明）
"""
import openpyxl
...
append_rows('多智能体协同', [
    ['9', 'node-coordinator 分布式节点协调器(新增)', '分布式任务调度、就近调度、算力负载均衡(含A800 GPU调度)、CI/CD门禁对接', '不产出教材正文, 无节点则任务悬置', '算力←2云+2硬+4台式机(A800训练); 版本←GitHub', '任务(章节编写/方证病案采集/模型训练/图谱注入) → 就近节点调度 + 状态回传', '只读任务队列与节点状态; 对接GitHub Actions', '任何节点不得"赢家通吃"; 调度须给出就近理由; A800训练任务与轻量任务错峰'],
    ['协作协议(qhzy增补): ⑤任务分发一律经 node-coordinator: 章节编写→台式机/浪潮硬②; 方证病案采集→台式机T1-T4; 模型训练(A800)→浪潮硬②; 批量生成→华为硬①容器; 公网服务→华为云①; 版本门禁→GitHub。⑥节点状态60s心跳上报, 调度决策留痕。', '', '', '', '', '', '', ''],
], ncols=8)
...
append_rows('可复用Skills', [
    ['tcm-distributed-node-orchestration', '岐黄智医分布式节点编排(方剂学版): 华为云①/GitHub/华为硬①/浪潮硬②(含A800)/四台式机的任务就近调度、方证病案采集、A800模型训练调度、CI/CD门禁对接。', '用户要求"分布式部署方剂学平台/方证病案采集/A800模型训练/多节点调度"时使用。', '① 读0分布式体系Sheet拓扑; ② 任务类型→节点映射(病案采集→台式机, 模型训练A800→浪潮硬②, 批量→华为硬①, 公网→华为云①); ③ 对接GitHub Actions门禁; ④ 心跳监控与故障降级; ⑤ A800训练任务错峰调度。', '浪潮地址192.168.0.102单址; 台式机IP待定不臆造; 方证病案数据须脱敏并标注来源; A800算力任务须声明GPU占用。', 'deploy-v3.sh健康检查200; 节点状态表与实机一致; A800训练日志可复盘。'],
], ncols=6)
...
append_rows('0改写说明', [
    ['版本变更', 'v2.0-qhzy (2026-08-08): ① 新增0分布式体系Sheet(2云+2硬+4台式机拓扑, 浪潮硬含A800 GPU); ② 第24章新增24.4节(岐黄智医分布式教学平台+方剂数据闭环); ③ ICD-11命名表增补平台db校准列(修正错码MB48.0/8B11/MD81/MD90/DD91.x/ME84.2); ④ 病证单元库增补脏腑/六经/舌脉三列(33 BZU); ⑤ 多智能体协同新增node-coordinator(A800训练调度); ⑥ 兼容性思维流新增第7局; ⑦ 可复用Skills新增分布式编排规格。'],
], ncols=2)
...
append_rows('平台建设与依赖', [
    ...
    ['节点', '方剂学平台承载', '数据流', '', '', ''],
    ['华为云服务器 114.115.211.254', '公网入口: 看板/SageAPI/PWA', '智能组方/方歌训练API对外', '', '', ''],
    ['GitHub qhzytcm/tcmP', '版本+CI/CD门禁(G1/G2/G3)', '推送→门禁→部署', '', '', ''],
    ['华为硬服务器 192.168.1.12', 'Gateway调度+容器批量生成', '114首方正文批量生成', '', '', ''],
    ['浪潮硬服务器 192.168.0.102 (A800)', '计算后端: 图谱+Embedding+方证对应/配伍机制模型训练', '方证病案数据训练', '', '', ''],
    ['台式机T1-T4', 'L4作者/审稿 + 方证病案/方歌数据采集', '病案/方歌数据→浪潮A800训练', '', '', ''],
], ncols=6)
