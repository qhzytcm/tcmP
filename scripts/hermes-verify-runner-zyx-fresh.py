# -*- coding: utf-8 -*-
"""runner-fresh: tempfile OS-safe 生成 hermes-verify- 脚本 → 运行 → 自动清理（ad-hoc）"""
import os, subprocess, sys, tempfile

BODY = r'''# -*- coding: utf-8 -*-
"""hermes-verify-zyx-v2-fresh.py — qhzy-中药学.xlsx (gen_zyx_stepE 完善后) 聚焦验证（ad-hoc）"""
...
for kw in ['华为云服务器','GitHub 仓库服务器','华为硬服务器','浪潮硬服务器(含A800)','台式机 T1-T4',
           '2云 + 2硬 + 4台式机','A800','192.168.0.102','智能鉴别','药性预测','node-coordinator','CI/CD']:
    check(f"0分布式体系含[{kw}]", kw in t)
check("无双址残留", '192.168.1.11' not in t and '双址' not in t)

# 3. 30.4 节 + 学时归属（gen_zyx_stepE 新增）
...
check("0目录含30.4小节", '30.4.1 2云+2硬+4台式机拓扑与数据流' in t)
...
# 5. BZU 11 列 + 溯源变体说明（gen_zyx_stepE 新增）
ws = wb['病证单元库']
bzu_n = c9 = c10 = c11 = 0
...
check("BZU含药名变体说明", '生地' in tbzu and '丹皮' in tbzu)
...
fd, tmp_path = tempfile.mkstemp(prefix='hermes-verify-zyx-v2-', suffix='.py', dir=tempfile.gettempdir())
