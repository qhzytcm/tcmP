...
BODY = r'''# -*- coding: utf-8 -*-
"""hermes-verify-hjnj-platform-v2.py — qhzy-黄帝内经 平台适配补丁 聚焦验证（ad-hoc）"""
import sys, re, sqlite3, openpyxl
...
check("8B11校准", 'ischaemic' in cal.get('8B11', ''), cal.get('8B11', ''))
...
for code in ['MB48.0', '8B11', 'MD81', 'DD91.2', 'ME84.2', 'ME06', 'MD12', '5A11', 'MG30']:
    ...
    row = cur.fetchone()
    check(f"db命中[{code}]", row is not None, f"{code} 未命中")
conn.close()

# 4. build_document 端到端（17 病证单元）
...
fd, tmp_path = tempfile.mkstemp(prefix='hermes-verify-hjnj-platform-v2-', suffix='.py', dir=tempfile.gettempdir())
