# -*- coding: utf-8 -*-
"""
Step C: qhzy-中医诊断学 引擎Sheet 增补 + 0改写说明升级 + 平台建设与依赖增补
1. ICD11统一命名: 增补平台db校准列（含错码修正 MB48.0/8B11/MD81/MD90/DD91.x/ME84.2）
2. 病证单元库: 增补 脏腑/六经/舌脉 三列（26 BZU）
3. 多智能体协同: 新增 node-coordinator 角色（8→9）
4. 兼容性思维流: 新增第7局（算力调度）
5. 可复用Skills: 新增 tcm-distributed-node-orchestration 规格（6→7）
6. 0改写说明: 版本升级 v1.0 → v2.0-qhzy + 分布式任务说明
7. 平台建设与依赖: 追加 分布式节点适配 说明
"""
import openpyxl, re
...
if hdr_row:
    # 该表结构: 平台病码|统一命名|ICD-11码|英文术语|状态依据 (5列), 追加第6列校准
...
# ═══ 3. 多智能体协同 新增 node-coordinator ═══
append_rows('多智能体协同', [
    ['9', 'node-coordinator 分布式节点协调器(新增)', '分布式任务调度、就近调度、算力负载均衡、CI/CD门禁对接', '不产出教材正文, 无节点则任务悬置', '算力←2云+2硬+4台式机; 版本←GitHub', '任务(章节编写/四诊数据采集/图谱注入) → 就近节点调度 + 状态回传', '只读任务队列与节点状态; 对接GitHub Actions', '任何节点不得"赢家通吃"; 调度须给出就近理由; 节点故障→自动降级迁移'],
    ['协作协议(qhzy增补): ⑤任务分发一律经 node-coordinator: 章节编写→台式机/浪潮硬②; 四诊数据采集→台式机T1-T4; 批量生成→华为硬①容器; 公网服务→华为云①; 版本门禁→GitHub。⑥节点状态60s心跳上报, 调度决策留痕。', '', '', '', '', '', '', ''],
], ncols=8)

# ═══ 4. 兼容性思维流 新增第7局 ═══
append_rows('兼容性思维流', [
    ['7', '本地 vs 云端 vs 台式机(算力调度)', '台式机抢服务器算力/本地无算力就没用', '就近调度、所长相加: 轻量任务就近台式机, 批量上华为硬①容器, 图谱训练上浪潮硬②, 公网服务上华为云①', '算力池化: 2云(华为云+GitHub) + 2硬(华为硬+浪潮硬) + 4台式机 各司其职', '0分布式体系 Sheet + node-coordinator 调度协议'],
    ['', '', '', '', '', ''],
...
append_rows('可复用Skills', [
    ['tcm-distributed-node-orchestration', '岐黄智医分布式节点编排(诊断学版): 华为云①/GitHub/华为硬①/浪潮硬②/四台式机的任务就近调度、四诊数据采集、CI/CD门禁对接。', '用户要求"分布式部署诊断学平台/四诊数据采集/多节点调度"时使用。', '① 读0分布式体系Sheet拓扑; ② 任务类型→节点映射(四诊采集→台式机, 图谱训练→浪潮硬②, 批量→华为硬①, 公网→华为云①); ③ 对接GitHub Actions门禁; ④ 心跳监控与故障降级。', '浪潮地址192.168.0.102单址; 台式机IP待定不臆造; 四诊数据须脱敏并标注来源。', 'deploy-v3.sh健康检查200; 节点状态表与实机一致; 四诊标注数据合规。'],
], ncols=6)
...
for r in range(1, ws.max_row + 1):
    for c in range(1, 5):
        ...
        if v and isinstance(v, str):
            ...
            if '版本 | v1.0' in nv:
                nv = '版本 | v2.0-qhzy (2026-08-08) — 由种子 v1.0 做分布式体系适配 + ICD-11 平台db校准'
            if nv != v:
                ws.cell(row=r, column=c, value=nv)
# 追加版本变更行
append_rows('0改写说明', [
    ['版本变更', 'v2.0-qhzy (2026-08-08): ① 新增0分布式体系Sheet(2云+2硬+4台式机拓扑); ② 14章新增14.4/14.5节(岐黄智医分布式教学平台+四诊客观化数据闭环); ③ ICD-11命名表增补平台db校准列(修正错码MB48.0/8B11/MD81/MD90/DD91.x/ME84.2); ④ 病证单元库增补脏腑/六经/舌脉三列; ⑤ 多智能体协同新增node-coordinator; ⑥ 兼容性思维流新增第7局; ⑦ 可复用Skills新增分布式编排规格。'],
], ncols=2)
...
append_rows('平台建设与依赖', [
    ...
    ['节点', '诊断学平台承载', '数据流', '', '', ''],
    ['华为云服务器 114.115.211.254', '公网入口: 看板/SageAPI/PWA', '四诊推理API对外', '', '', ''],
    ['GitHub qhzytcm/tcmP', '版本+CI/CD门禁(G1/G2/G3)', '推送→门禁→部署', '', '', ''],
    ['华为硬服务器 192.168.1.12', 'Gateway调度+容器批量生成', '章节批量生成', '', '', ''],
    ['浪潮硬服务器 192.168.0.102', '计算后端: 图谱+Embedding+舌诊脉象模型', '四诊数据训练', '', '', ''],
    ['台式机T1-T4', 'L4作者/审稿 + 四诊客观化数据采集', '舌象/脉象/问诊数据→浪潮训练', '', '', ''],
], ncols=6)
